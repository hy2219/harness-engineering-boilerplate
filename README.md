...
.github/
  ...
  workflows/
    self-heal.yml            ← CI/CD 실패 → 자동 수정 Issue 생성
    watchdog.yml             ← 미할당 self-heal Issue 감시 + Slack 알림
    approval-loop.yml        ← Self-heal PR 머지 → Issue 닫기 + 이력 업데이트
    slack-notify.yml         ← PR/워크플로우 결과 Slack 알림
...
이 보일러플레이트에는 Harness Engineering 자동화 워크플로우 4개가 포함되어 있습니다.
Copilot이 [구현] 단계에서 CI/CD/Infra 워크플로우를 생성하면 자동으로 연동됩니다.

| 워크플로우 | 트리거 | 역할 |
|-----------|--------|------|
| `self-heal.yml` | CI/CD/Infra 실패 시 | 에러 로그 수집 → `[Self-Heal]` Issue 자동 생성 → 이력 기록 |
| `watchdog.yml` | 30분마다 | 미할당 self-heal Issue 감시 → 1시간 초과 시 리마인더 + Slack 알림 |
| `approval-loop.yml` | Self-heal PR 머지 시 | Issue 닫기 + `docs/self-heal-log.md` 상태 업데이트 |
| `slack-notify.yml` | PR 생성 / 워크플로우 완료 시 | Slack 채널에 결과 알림 (선택 — Webhook 등록 시 활성화) |

> **참고**: `self-heal.yml`과 `slack-notify.yml`은 `"CI — Test & Lint"`, `"CD — Deploy to Azure"`, `"Infra — Provision Azure Resources"` 이름의 워크플로우를 감시합니다. Copilot이 [구현] 시 이 이름 규칙을 따릅니다.
